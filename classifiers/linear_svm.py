...
def svm_loss_naive(W, X, y, reg):
  ...
  for i in xrange(num_train):
    ...
    for j in xrange(num_classes):
      ...
      # L>0 일때 미분값 생각해서 L= 1(margin >0) x_i
      if margin > 0:
        loss += margin
        dW[:,j] +=X[i,:]
        dW[:,y[i]] -= X[i,:] 
...
def svm_loss_vectorized(W, X, y, reg):
  ...
  delta[np.arange(num_train),y]=0 # do not consider correct class in loss

  correct_class_score = np.reshape(correct_class_score,(num_train,1))
  margin = np.maximum(0, scores - correct_class_score +1) # 각 행으로 빼기 연산
  ...
  valid_margin_count = margin.sum(axis =1) #(N,)
